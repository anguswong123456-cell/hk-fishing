...
app.get('/api/tide', async (req, res) => {
  try {
    ...
    if (processedData.length === 0) {
      processedData = [
        { location: '橫瀾島', time: '06:00', height: 1.2, unit: 'm', status: '漲潮中', biteScore: Math.min(10, Math.max(1, 7 + weatherBonus)), pressure: pressure + ' hPa', weatherCondition: weatherCondition },
        { location: '橫瀾島', time: '11:30', height: 2.1, unit: 'm', status: '滿潮 (High Tide)', biteScore: Math.min(10, Math.max(1, 9 + weatherBonus)), pressure: pressure + ' hPa', weatherCondition: weatherCondition },
        { location: '橫瀾島', time: '17:15', height: 0.6, unit: 'm', status: '乾潮 (Low Tide)', biteScore: Math.min(10, Math.max(1, 4 + weatherBonus)), pressure: pressure + ' hPa', weatherCondition: weatherCondition }
      ];
    }
